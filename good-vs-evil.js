...
function goodVsEvil(good, evil){
  let goodArray = good.split(' '), goodValues = [1, 2, 3, 3, 4, 10];
  let evilArray = evil.split(' '), evilValues = [1, 2, 2, 2, 3, 5, 10];
  let countForces = (arr, val) => {
    let total = 0;
    for (let i = 0; i < arr.length; i++) total += val[i] * parseInt(arr[i]);
    return total;
  }
  let goodForces = countForces(goodArray, goodValues);
  let evilForces = countForces(evilArray, evilValues);
  return goodForces > evilForces ? 'Battle Result: Good triumphs over Evil'
  : goodForces < evilForces ? 'Battle Result: Evil eradicates all trace of Good'
